...
- 数据库迁移文件统一放在 `apps/server/database/migrations/`
- 迁移文件只保留当前结构所需的版本，不保留被后续版本替代的中间产物
- 新功能的建表、索引和约束放在该功能自己的迁移中，不植入其他模块的初始化迁移
- 跨模块追加结构时，新增后续迁移；按业务依赖顺序排列，不篡改先前的初始化迁移
- 同一模块内被后续迁移整体替代的中间迁移，应先合并最终结构再删除
- 删除中间迁移前，必须确认其结构已完整并入保留的迁移，且没有被其他迁移依赖
