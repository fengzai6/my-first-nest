...
- **说明"为什么"而非"是什么"**：注释服务于读代码的人理解上下文，而不是复述代码行为。着重说明设计权衡、边界约束、被否掉的替代方案和踩过的坑。
- **标记坑位与隐式约定**：遇到容易误用的 API、时序依赖、隐式假设或非显而易见的边界情况，使用 `// NOTE:`、`// HACK:` 或 `// FIXME:` 标注理由，让后来者不会重复踩坑。
- **记录决策上下文**：当选择了一个有争议或多种可行方案的实现路径时，在代码附近留下简洁的决策理由和当时被否的方案概要，方便学习者理解演进过程。
- **功能交付后沉淀学习笔记**：每个功能完成后，评估是否需要在 `docs/` 或相应 README 中记录设计思路、关键决策和注意事项，供团队和将来者参考。
- **不写冗余注释**：不注释自明代码（直白的方法调用、明显的循环等）。注释只承担代码无法表达的信息——理由、坑、约束、上下文。
