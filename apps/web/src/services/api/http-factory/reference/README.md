...
- **`skipRefreshUrls`** — 不触发 refresh token 流程的请求 URL 列表
...
- **`authFailureCodes`** — 业务状态码中用于识别鉴权失败的 code 列表
...
- **`isRefreshFailure`** — 判断刷新请求是否已经失败到需要退出登录，默认行为：
  ...
  - 命中 `unauthorizedStatusCode` 或 `authFailureCodes` → 视为鉴权失败
...
8. refresh 错误命中 authFailureCodes 时会视为登录过期
...
- `http-client.refresh.test.ts` — shouldSkipRefresh（undefined config/url）、defaultIsRefreshFailure（非 AxiosError/无 response/5xx/authFailureCodes）
